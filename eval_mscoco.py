...
log_file_path = os.path.join(log_dir, f"eval_mscoco_{int(time.time())}.log")
...
def initialize_retrieval_worker(args):
    ...
    try:
        ...
        worker_retrieval_model.get_image_embeddings(image_files, "mscoco_5k_test")
...
class MscocoEvaluationPipeline:
    ...
    def _prepare_data(self):
        ...
        if not os.path.isdir(image_dir) or not os.path.isfile(csv_path):
            ...
            return None, None
        logger.info(f"Loading data from local CSV: {csv_path}")
        df = pd.read_csv(csv_path)
        for key in ['sentids', 'imgid', 'tokens', 'raw', 'sentid']:
            if key in df.columns: df[key] = df[key].apply(json.loads)
    ...
    def evaluate(self):
        ...
        initial_results_path = os.path.join(output_dir, f"MSCOCO_ViT_L14_initial_retrieval_results_{self.model_name}.json")
        final_results_path = os.path.join(output_dir, f"final_results_ViT_L14_{mode_str}.json")
        ...
        if os.path.exists(initial_results_path):
            ...
            with open(initial_results_path, 'r') as f: initial_results = json.load(f)
        else:
            logger.info("Pre-warming image embedding cache...")
            try:
                image_paths = [os.path.join(image_dir_path, q['image_id']) for q in text_queries]
                main_process_model = RetrievalModel(self.model_name, config.BLIP_ITR_MODEL_PATH)
                main_process_model.get_image_embeddings(sorted(list(set(image_paths))), "mscoco_5k_test")
            ...
            logger.info(f"Performing initial retrieval with {self.args.num_workers} workers...")
            initial_results = []
            with multiprocessing.Pool(self.args.num_workers, initialize_retrieval_worker, (self.args,)) as pool:
                with tqdm(total=len(text_queries), desc="Initial Retrieval") as pbar:
                    for result in pool.imap_unordered(initial_retrieval_worker, text_queries):
                        if result: initial_results.append(result)
                        pbar.update(1)
            self._save_json_results(initial_results_path, initial_results)
            logger.info(f"Initial retrieval complete. Results saved to {initial_results_path}")
        ...
        header = f"\n{'='*20} MSCOCO 5k Test Results ({mode_str}) {'='*20}"
        logger.info(header)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run evaluation on MSCOCO 5k test set.")
    parser.add_argument('--model', type=str, default='CLIP', choices=['BLIP', 'CLIP'])
    parser.add_argument('--rerank', action='store_true')
    parser.add_argument('--num-workers', type=int, default=8, help='Workers for initial retrieval.')
    parser.add_argument('--rerank-workers', type=int, default=19, help='Workers for reranking.')
    parser.add_argument('--top-k', type=int, default=20, help='Top K IDs to save in JSON output.')
    parser.add_argument('--csv_path', type=str, required=True, help='Path to the local test CSV file.')
    parser.add_argument('--image_dir', type=str, required=True, help='Path to the local image directory.')
